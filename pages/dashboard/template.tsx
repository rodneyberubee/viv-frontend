...
const DashboardTemplate = ({ restaurantId }: DashboardProps) => {
  ...
  const [selectedRows, setSelectedRows] = useState<string[]>([]); // store IDs for selection
  ...
  useEffect(() => {
    ...
    async function exchangeForJwt(tempToken: string) {
      try {
        const res = await fetch('https://api.vivaitable.com/api/auth/login/verify', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ token: tempToken }) });
      ...
      } catch (err) { console.error('[ERROR] Token exchange failed:', err); localStorage.removeItem('jwtToken'); window.location.href = '/login'; }
  ...
  async function fetchConfig() {
    if (!jwtToken) return;
    try { const res = await safeFetch(`https://api.vivaitable.com/api/dashboard/${restaurantId}/config`, { headers: { Authorization: `Bearer ${jwtToken}` } }); const data = await res.json(); setConfig(data.config || data); }
  ...
  const handleReservationEdit = (e: React.ChangeEvent<HTMLInputElement>, id: string) => { const { name, value } = e.target; setReservations((prev) => prev.map((res) => (res.id === id ? { ...res, [name]: value } : res))); };
  ...
  const addNewRow = async () => {
    ...
    } catch (err) { console.error('[ERROR] Adding new row failed:', err); }
  };

  // Mark selected rows as hidden
  const deleteSelectedRows = async () => {
    ...
    try {
      ...
      await fetchReservations();
      setSelectedRows([]);
    } catch (err) { console.error('[ERROR] Hiding rows failed:', err); }
  };
  ...
  const updateConfig = async () => {
    ...
    try { await safeFetch(`https://api.vivaitable.com/api/dashboard/${restaurantId}/updateConfig`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${jwtToken}` }, body: JSON.stringify({ ...cleaned, restaurantId }) }); alert('Config updated'); }
  ...
  const updateReservations = async () => {
    ...
    try {
      ...
      await safeFetch(`https://api.vivaitable.com/api/dashboard/${restaurantId}/updateReservation`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${jwtToken}` }, body: JSON.stringify(payload) });
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      ...
      <main className="flex-1 p-8 space-y-8">
        ...
        {/* Config section remains unchanged */}
        <section className="bg-white rounded shadow p-6">
          ...
          <div className="mt-4 overflow-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50"><tr>{['monday','tuesday','wednesday','thursday','friday','saturday','sunday'].map((day) => (<th key={day} className="px-3 py-2 text-left text-gray-700 font-medium capitalize">{day}</th>))}</tr></thead>
              <tbody>
                <tr className="hover:bg-gray-50">{['monday','tuesday','wednesday','thursday','friday','saturday','sunday'].map((day) => (<td key={day+'Open'} className="px-3 py-2 border-t"><input type="text" placeholder="HH:mm or HH:mm AM/PM" name={`${day}Open`} value={config[`${day}Open`] || ''} onChange={handleConfigChange} className="w-full p-1 border rounded" /></td>))}</tr>
                <tr className="hover:bg-gray-50">{['monday','tuesday','wednesday','thursday','friday','saturday','sunday'].map((day) => (<td key={day+'Close'} className="px-3 py-2 border-t"><input type="text" placeholder="HH:mm or HH:mm AM/PM" name={`${day}Close`} value={config[`${day}Close`] || ''} onChange={handleConfigChange} className="w-full p-1 border rounded" /></td>))}</tr>
              </tbody>
            </table>
          </div>
